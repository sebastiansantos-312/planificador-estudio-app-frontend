 ...
 *
 * Cambio Sprint 1: se agrega selector de tipo de actividad (US-01).
 */
...
import type { Subject, TaskPriority, TaskType, SubtaskCreate, LoadingState } from "../types";
...
export default function CrearPage() {
    ...
    const [taskType, setTaskType] = useState<TaskType | "">("");  // ← NUEVO (US-01)
    ...
    async function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        if (!session || !subjectId || !taskType) return;
        setErrorMsg("");
            ...
            <form onSubmit={handleSubmit} className="space-y-6" noValidate>
                ...
                <fieldset className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
                    ...
                    <div>
                        ...
                        {/* Mensaje de ayuda cuando no hay tipo seleccionado */}
                        {!taskType && (
                ...
                {/* Botón submit — deshabilitado si faltan campos obligatorios */}
                <button
                    type="submit"
                    disabled={isLoading || !title.trim() || !taskType || !subjectId || !dueDate}
